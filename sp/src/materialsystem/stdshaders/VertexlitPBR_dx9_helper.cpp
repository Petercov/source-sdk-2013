							...
							CBasePerMaterialContextData **pContextDataPtr )
{
	...
	else // not snapshotting -- begin dynamic state
	{
		...
		SET_DYNAMIC_PIXEL_SHADER_COMBO( LIGHTMAP, bHasLightmap && !bHasFlashlight);
		...
		SET_DYNAMIC_PIXEL_SHADER_COMBO(CSM_PERF, 0);// MAX(0, MIN(r_csm_performance.GetInt(), 2)) ); // i just dont know anymore
