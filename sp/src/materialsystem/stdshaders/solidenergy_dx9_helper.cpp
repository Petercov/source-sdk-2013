					...
					CBasePerMaterialContextData **pContextDataPtr )
{
	...
	bool bHasCheapFlow = !( IsX360() ) && bHasFlowmap && ( params[info.m_nFlowCheap]->GetIntValue() != 0 );
	...
	if ( pShader->IsSnapshotting() || (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) )
	{
		...
		if ( pShader->IsSnapshotting() )
		{
...
#endif
			{
				DECLARE_STATIC_VERTEX_SHADER_NEW( solidenergy_vs20 );
				SET_STATIC_VERTEX_SHADER_COMBO_NEW( VERTEXCOLOR,  bHasVertexColor || bHasVertexAlpha );
				SET_STATIC_VERTEX_SHADER_COMBO_NEW( DETAIL1, bDetail1 );
				SET_STATIC_VERTEX_SHADER_COMBO_NEW( DETAIL2, bDetail2 );
				SET_STATIC_VERTEX_SHADER_COMBO_NEW( TANGENTTOPACITY, bTangentT );
				SET_STATIC_VERTEX_SHADER_COMBO_NEW( TANGENTSOPACITY, bTangentS );
			...
			{
				DECLARE_STATIC_VERTEX_SHADER_NEW( solidenergy_vs30 );
				SET_STATIC_VERTEX_SHADER_COMBO_NEW( VERTEXCOLOR,  bHasVertexColor || bHasVertexAlpha );
				SET_STATIC_VERTEX_SHADER_COMBO_NEW( DETAIL1, bDetail1 );
				SET_STATIC_VERTEX_SHADER_COMBO_NEW( DETAIL2, bDetail2 );
				SET_STATIC_VERTEX_SHADER_COMBO_NEW( TANGENTTOPACITY, bTangentT );
...
#endif
			{
				DECLARE_STATIC_PIXEL_SHADER_NEW( solidenergy_ps20b );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( ADDITIVE, bAdditiveBlend );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( DETAIL1, bDetail1 );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( DETAIL2, bDetail2 );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( DETAIL1BLENDMODE, nDetail1BlendMode );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( DETAIL2BLENDMODE, nDetail2BlendMode );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( VERTEXCOLOR, ( bHasVertexColor || bHasVertexAlpha ) );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( TANGENTTOPACITY, bTangentT );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( TANGENTSOPACITY, bTangentS );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( FRESNELOPACITY, bFresnel );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( DEPTHBLEND, bDepthBlend );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( FLOWMAP, bHasFlowmap );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( FLOW_CHEAP, bHasCheapFlow );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW(REFRACT, bDoRefract);
				SET_STATIC_PIXEL_SHADER_NEW( solidenergy_ps20b );
			}
#if !defined( _GAMECONSOLE )
			else
			{
				DECLARE_STATIC_PIXEL_SHADER_NEW( solidenergy_ps30 );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( ADDITIVE, bAdditiveBlend );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( DETAIL1, bDetail1 );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( DETAIL2, bDetail2 );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( DETAIL1BLENDMODE, nDetail1BlendMode );
				...
				SET_STATIC_PIXEL_SHADER_COMBO_NEW( FLOW_CHEAP, bHasCheapFlow );
				SET_STATIC_PIXEL_SHADER_COMBO_NEW(REFRACT, bDoRefract);
				SET_STATIC_PIXEL_SHADER_NEW( solidenergy_ps30 );
			}
#endif
...
//#endif
	...
	if ( pShaderAPI ) //DYNAMIC_STATE
	{
...
#endif
		{
			DECLARE_DYNAMIC_VERTEX_SHADER_NEW( solidenergy_vs20 );
		...
		{
			DECLARE_DYNAMIC_VERTEX_SHADER_NEW( solidenergy_vs30 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_NEW( COMPRESSED_VERTS, (int)vertexCompression );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_NEW( SKINNING,  pShaderAPI->GetCurrentNumBones() > 0 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_NEW( VORTEX1, bVortex1 );
			SET_DYNAMIC_VERTEX_SHADER_COMBO_NEW( VORTEX2, bVortex2 );
		...
		ITexture* pRT = pShaderAPI->GetRenderTargetEx(0);
		if (pRT)
		{
			nRtHeight = pRT->GetActualHeight();
			nRtWidth = pRT->GetActualWidth();
		}
		else
		{
		...
		/*flConsts[0] =  .5f * ( ( float )viewport.m_nWidth / ( float )nRtWidth );
		flConsts[1] = -.5f * ( ( float )viewport.m_nHeight / ( float )nRtHeight );
		flConsts[2] =  flConsts[0] + ( ( float )viewport.m_nTopLeftX / ( float )nRtWidth );
		flConsts[3] = -flConsts[1] + ( ( float )viewport.m_nTopLeftY / ( float )nRtHeight );
...
#endif
		{
			DECLARE_DYNAMIC_PIXEL_SHADER_NEW( solidenergy_ps20b );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_NEW( ACTIVE, bActive );
		...
		{
			DECLARE_DYNAMIC_PIXEL_SHADER_NEW( solidenergy_ps30 );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_NEW( ACTIVE, bActive );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_NEW( POWERUP, bPowerup );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_NEW( VORTEX1, bVortex1 );
			SET_DYNAMIC_PIXEL_SHADER_COMBO_NEW( VORTEX2, bVortex2 );
