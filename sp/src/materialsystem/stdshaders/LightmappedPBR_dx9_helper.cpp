							...
							CBasePerMaterialContextData **pContextDataPtr )
{
	...
	if( pShader->IsSnapshotting() )
	{
		...
		DECLARE_STATIC_VERTEX_SHADER_NEW( lightmappedPBR_vs30 );
		SET_STATIC_VERTEX_SHADER_COMBO_NEW(VERTEXCOLOR, bHasVertexColor || bHasVertexAlpha);
		SET_STATIC_VERTEX_SHADER_COMBO_NEW(BUMPMAP, bHasBump);
		SET_STATIC_VERTEX_SHADER_COMBO_NEW(DIFFUSEBUMPMAP, bHasBump);
		SET_STATIC_VERTEX_SHADER_COMBO_NEW(VERTEXALPHATEXBLENDFACTOR, false);
		SET_STATIC_VERTEX_SHADER_NEW(lightmappedPBR_vs30);

		// Assume we're only going to get in here if we support 2b
		DECLARE_STATIC_PIXEL_SHADER_NEW(lightmappedPBR_ps30);
		...
		SET_STATIC_PIXEL_SHADER_COMBO_NEW(SEAMLESS, false);
		SET_STATIC_PIXEL_SHADER_COMBO_NEW(BUMPMAP, bHasBump);
#ifdef PARALLAX_CORRECTED_CUBEMAPS
		// Parallax cubemaps enabled for 2_0b and onwards
		SET_STATIC_PIXEL_SHADER_COMBO_NEW(PARALLAXCORRECT, hasParallaxCorrection);
#else
		SET_STATIC_PIXEL_SHADER_COMBO_NEW(PARALLAXCORRECT, false);
#endif
		...
		SET_STATIC_PIXEL_SHADER_NEW(lightmappedPBR_ps30);
	...
	else // not snapshotting -- begin dynamic state
	{
		...
		DECLARE_DYNAMIC_VERTEX_SHADER_NEW( lightmappedPBR_vs30 );
		SET_DYNAMIC_VERTEX_SHADER_COMBO_NEW( DOWATERFOG, fogIndex );
		SET_DYNAMIC_VERTEX_SHADER_COMBO_NEW( FASTPATH, true);
		SET_DYNAMIC_VERTEX_SHADER_NEW(lightmappedPBR_vs30);

		DECLARE_DYNAMIC_PIXEL_SHADER_NEW(lightmappedPBR_ps30);
		...
		if( !bHasFlashlight )
		{
			...
			if (hasParallaxCorrection)
			{
				...
				matrix[3][3] = 1;
				pShaderAPI->SetPixelShaderConstant(34, &matrix[0][0], 4);
			}
#endif
