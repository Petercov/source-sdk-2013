...
BEGIN_DATADESC( CNPC_WScanner )
	...
	DEFINE_KEYFIELD( m_nPodSize,					FIELD_INTEGER, "NumberOfPods" ),

	DEFINE_FIELD(	m_hHelper,					FIELD_EHANDLE),
	DEFINE_FIELD(	m_nWScannerState,			FIELD_INTEGER),
	DEFINE_ARRAY(	m_pBeam,					FIELD_CLASSPTR,		WSCANNER_NUM_BEAMS ),
	DEFINE_ARRAY(	m_pLaserGlow,				FIELD_CLASSPTR,		WSCANNER_NUM_GLOWS ),
	...
	DEFINE_FIELD( m_fStartJoinTime,			FIELD_TIME),
...
// Output :
//-----------------------------------------------------------------------------
void CNPC_WScanner::TraceAttack(const CTakeDamageInfo& info, const Vector& vecDir, trace_t* ptr, CDmgAccumulator* pAccumulator)
{
...
void CNPC_WScanner::Precache(void)
{
	...
	// Model.
	//
	PrecacheModel("models/wscanner_single.mdl");
	PrecacheModel("models/wscanner_pair.mdl");
	PrecacheModel("models/wscanner_tri.mdl");
...
void CNPC_WScanner::RunTask( const Task_t *pTask )
{
	switch ( pTask->iTask )
	{
		...
		case TASK_WAIT_FOR_MOVEMENT:
		{
			...
			else if (IsCurSchedule(SCHED_WSCANNER_GET_HELP) && GetHintNode())
			{
				Vector vHintPos;
				GetHintNode()->GetPosition(this,&vHintPos);
