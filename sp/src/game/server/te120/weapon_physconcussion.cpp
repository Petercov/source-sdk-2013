...
void CWeaponPhysConcussion::CalcClip()
{
	...
	if ( m_iClip1 < 100 || pPlayer->SuitPower_GetCurrentPercentage() < 100.0 )
	{
		float fClip = clamp( pPlayer->SuitPower_GetCurrentPercentage() - (m_flEnergyRequired * (1 - (float) m_bOpen)), (float) m_bOpen, 100.0f );
		float fOverHeatFactor = 1 - ((m_flEnergyRequired - MIN_ENERGY_REQUIRED) / (MAX_ENERGY_REQUIRED - MIN_ENERGY_REQUIRED));
...
void CWeaponPhysConcussion::PrimaryAttack( void )
{
	...
	float fTotalDelay = clamp(	(clamp( m_flEnergyRequired - pPlayer->SuitPower_GetCurrentPercentage(), 0.1, 100.0 ) / 12.5f) + m_flOverHeatWait,
								MIN_READY_DELAY,
	...
	m_flEnergyRequired += pow( m_flEnergyRequired * physconcussion_energypowscale.GetFloat(), 2 );
	m_flEnergyRequired = clamp( m_flEnergyRequired, MIN_ENERGY_REQUIRED, MAX_ENERGY_REQUIRED );
	// Msg( "Next Energy Required: %f\n\n", m_flEnergyRequired );

	// Calculate next ready delay
	m_flOverHeatWait += pow( m_flOverHeatWait, physconcussion_mindelaypower.GetInt() );
	m_flOverHeatWait = clamp( m_flOverHeatWait, MIN_READY_DELAY, MAX_READY_DELAY );
	// Msg( "Next Ready Delay: %f\n\n", m_flOverHeatWait );

	m_flNextCoolDown = gpGlobals->curtime + 2.0f;
	m_flLastRecoveryTime = 1.0;
	m_flRecoveryRateScale *= m_flShrinkRate;
...
void CWeaponPhysConcussion::ItemPreFrame()
{
	...
	if (gpGlobals->curtime >= (m_flNextPrimaryAttack - 0.3) )
	{
		...
		{
			if (pPlayer->SuitPower_GetCurrentPercentage() >= m_flEnergyRequired)
			{
				if ( !m_bOpen )
					pPlayer->SuitPower_Drain(m_flEnergyRequired);
