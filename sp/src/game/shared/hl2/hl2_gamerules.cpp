...
ConVar  alyx_darkness_force( "alyx_darkness_force", "0", FCVAR_CHEAT | FCVAR_REPLICATED );
	...
	void CHalfLife2::InitDefaultAIRelationships( void )
	{
		...
		for (i = 0; i < nClasses; i++)
		{
			...
			{
				// By default we like ourselves, are neutral to bullseyes, and hate everything else.
				if (i == j)
				{
				...
				else if (j == CLASS_NONE || j == CLASS_BULLSEYE || j == CLASS_FLARE || j == CLASS_EARTH_FAUNA)
				{
					CBaseCombatCharacter::SetDefaultRelationship((Class_T)i, (Class_T)j, D_NU, 0);
				}
				else
...
#endif
#if defined(HL1_NPCS) || defined(EZ_NPCS)
