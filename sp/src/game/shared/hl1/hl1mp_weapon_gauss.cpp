...
#else
extern ConVar hl1_sk_plr_dmg_gauss;
#define sk_plr_dmg_gauss hl1_sk_plr_dmg_gauss
#endif // !MAPBASE
...
void CWeaponGauss::Precache( void )
{
	...
	PrecacheScriptSound( "HL1Weapon_Gauss.Zap1" );
	PrecacheScriptSound( "HL1Weapon_Gauss.Zap2" );
	PrecacheScriptSound( "HL1Weapon_Gauss.Fire" );
	PrecacheScriptSound( "HL1Weapon_Gauss.StaticDischarge" );
	PrecacheScriptSound( "HL1Weapon_Gauss.Spin" );
...
void CWeaponGauss::SecondaryAttack( void )
{
	...
	if ( pPlayer->GetWaterLevel() == 3 )
	{
		if ( m_nAttackState != 0 )
		{
			EmitSound( "HL1Weapon_Gauss.Zap1" );
...
//FIXME		pPlayer->m_iWeaponVolume = GAUSS_PRIMARY_CHARGE_VOLUME;
		...
		if ( m_sndCharge == NULL )
		{
			...
			m_sndCharge	= (CSoundEnvelopeController::GetController()).SoundCreate( filter, entindex(), CHAN_WEAPON, "HL1Weapon_Gauss.Spin", ATTN_NORM );
		}
...
//FIXME		m_pPlayer->m_iWeaponVolume = GAUSS_PRIMARY_CHARGE_VOLUME;
		...
		if ( m_flStartCharge < gpGlobals->curtime - 10 )
		{
			...
			EmitSound( "HL1Weapon_Gauss.Zap1" );
			EmitSound( "HL1Weapon_Gauss.Zap2" );
...
//			data6.m_nEntIndex	= pPlayer->GetViewModel()->entindex();
	...
	if ( GetParametersForSound( "HL1Weapon_Gauss.Fire", params, NULL ) )
	{
...
void CWeaponGauss::WeaponIdle( void )
{
	...
	{
		EmitSound( "HL1Weapon_Gauss.StaticDischarge" );
